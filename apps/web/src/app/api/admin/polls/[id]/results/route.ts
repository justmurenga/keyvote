...
// GET - Get poll results breakdown by region with drill-down support
export async function GET(
  ...
  try {
    ...
    const { data: poll, error: pollError } = await supabase
      ...
      .eq('id', id)
      .single() as { data: { id: string; title: string; position: ElectoralPosition } | null; error: any };
    ...
    // Demographics
    const genderResults: Record<string, number> = {};
    const ageResults: Record<string, number> = {};
    ...
    votes?.forEach(vote => {
      ...
      if (vote.voter_gender) {
        genderResults[vote.voter_gender] = (genderResults[vote.voter_gender] || 0) + 1;
      }

      // Age bracket aggregation
      if (vote.voter_age_bracket) {
        ageResults[vote.voter_age_bracket] = (ageResults[vote.voter_age_bracket] || 0) + 1;
      }
    });
    ...
    return NextResponse.json({
      ...
      byConstituency,
      byWard,
      byGender: Object.entries(genderResults).map(([gender, voteCount]) => ({
        gender,
        votes: voteCount,
        percentage: totalVotes > 0 ? (voteCount / totalVotes) * 100 : 0,
      })),
      byAge: Object.entries(ageResults).map(([age, voteCount]) => ({
        age,
        votes: voteCount,
        percentage: totalVotes > 0 ? (voteCount / totalVotes) * 100 : 0,
      })),
