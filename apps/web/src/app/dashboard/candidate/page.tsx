...
function CandidateDashboardInner() {
  ...
  const initialTab = searchParams?.get('tab') === 'edit' ? 'edit' : 'overview';
  ...
  useEffect(() => {
    const next = searchParams?.get('tab') === 'edit' ? 'edit' : 'overview';
    setTab(next);
  }, [searchParams]);
  ...
  const handleTabChange = (next: string) => {
    ...
    const base = pathname ?? '/dashboard/candidate';
  ...
  return (
    <div className="space-y-6">
      {/* Engagement banner — visible at top once approved */}
      <CandidateEngagementBanner />

      {/* Header */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold">My Campaign</h1>
          <p className="text-muted-foreground">
      ...
      <Tabs value={tab} onValueChange={handleTabChange} className="space-y-6">
              ...
              <CardContent className="flex items-center gap-4 py-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-yellow-700 dark:text-yellow-300">
                    Your candidate profile is being reviewed by our admin team. You can still set
                    up your campaign while waiting.
                  </p>
                </div>
              </CardContent>
            </Card>
        ...
        <TabsContent value="edit" className="mt-0">
          <CandidateProfileForm onSaved={fetchCandidateProfile} />
        </TabsContent>
      </Tabs>
    </div>
  );
